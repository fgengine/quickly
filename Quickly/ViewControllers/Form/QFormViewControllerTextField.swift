...
open class QFormViewControllerTextField : QFormViewControllerField {
    ...
    public private(set) lazy var inputFieldView: QTextField = {
        ...
        view.onShouldClear = { [weak self] textField -> Bool in return self?.fieldShouldClear() ?? true }
        view.onPressedClear = { [weak self] textField in self?.fieldPressedClear() }
        view.onShouldReturn = { [weak self] textField -> Bool in return self?.fieldShouldReturn() ?? true }
        view.onPressedReturn = { [weak self] textField in self?.fieldPressedReturn() }
        self.inputView.addSubview(view)
        return view
    }()
    ...
    open func fieldShouldClear() -> Bool {
        return true
    }
    
    open func fieldPressedClear() {
    }
    
    open func fieldShouldReturn() -> Bool {
        return true
    }

    open func fieldPressedReturn() {
    }
